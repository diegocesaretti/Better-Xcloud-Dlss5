...
function Set-IniKey([string]$Text, [string]$Section, [string]$Key, [string]$Value) {
    ...
    if (-not $sectionMatch.Success) {
        ...
        return $Text + "[$Section]`r`n$Key=$Value`r`n"
    }
...
try {
    ...
    Configure-NeuralRuntime $installedNeural
...
,
    ...
    if ($keyRegex.IsMatch($block)) {
        $block = $keyRegex.Replace($block, "$Key=$Value", 1)
    } else {
...
function Configure-NeuralRuntime([string]$NeuralDirectory) {
    $iniPath = Join-Path $NeuralDirectory 'ReShade.ini'
    if (-not (Test-Path -LiteralPath $iniPath)) {
        Fail 'ReShade.ini disappeared while configuring the neural runtime.' 23
    }

    $text = [IO.File]::ReadAllText($iniPath)

    # ReShade disables add-ons through a comma-separated list. Remove every
    # known spelling of the RenoDX neural add-on while preserving other entries.
    $disabledRegex = [regex]::new(
        '^DisabledAddons=(.*)
if (-not [Environment]::Is64BitOperatingSystem) {
    Fail 'Windows x64 is required.' 10
}
...
,
    ...
    $text = $disabledRegex.Replace($text, {
        param($match)
        $blocked = @(
        ...
        $items = @($match.Groups[1].Value -split ',' | ForEach-Object { $_.Trim() })
        $kept = @($items | Where-Object { $_ -and ($_ -notin $blocked) })
        return 'DisabledAddons=' + ($kept -join ',')
    })

    # Match the upstream helper bootstrap contract. EnableHooks=2 is the
    # raw-NGX-only path used by this bridge.
    $text = Set-IniKey $text 'RenoDX.DLSS5' 'EnableHooks' '2'
    $text = Set-IniKey $text 'RenoDX.DLSS5' 'NeuralUplift' '1'
    $text = Set-IniKey $text 'RenoDX.DLSS5' 'NREnableUpscaling' '0'
